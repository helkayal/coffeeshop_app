...
class LoyaltyProgressBar extends StatelessWidget {
    ...
    return Column(children: [
      LayoutBuilder(builder: (context, constraints) {
        ...
        return SizedBox(height: 36, child: Stack(clipBehavior: Clip.none, children: [
          PositionedDirectional(start: 0, top: 0, child: _track(w, cs.surfaceContainerHighest)),
          PositionedDirectional(start: 0, top: 0, child: _track(w * progress, cs.primary)),
          ...
          PositionedDirectional(start: (w - cupW) * progress, top: -10, child: LoyaltyCupIndicator(color: cs.primary)),
        ]));
      }),
  ...
  Widget _track(double width, Color color) {
    return Container(width: width, height: 3, decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(1.5)));
  }
  ...
  Widget _dot(double centerX, double size, bool completed, ColorScheme cs) {
    return PositionedDirectional(
      ...
      child: Container(width: size, height: size,
          decoration: BoxDecoration(color: completed ? cs.primary : cs.surfaceContainerHighest, shape: BoxShape.circle)),
    );
  }
}
